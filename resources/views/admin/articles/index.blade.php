                                ...
                                <div class="form-group">
                                    <div class="input-group mb-4">
                                        <select class="form-control" name="admin_global_search">
                                            ...
                                            <option value="name" {{ request()->get("admin_global_search") == 'name' ? 'selected' : null }}>Name</option>
                                            <option value="description" {{ request()->get("admin_global_search") == 'description' ? 'selected' : null }}>Description</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    <div class="input-group mb-4">
                                        <select class="form-control" name="category_id">
                                            ...
                                            <option value="{{ $value->id }}" {{ request()->filled('category_id') && (int)request()->get('category_id') === $value->id ? 'selected':'' }}>{{ $value->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                ...
                <div class="card-header border-0">
                    <div class="row align-items-center">
                        ...
                        <div class="col-7 text-right">
                            ...
                            <a href="{{ route('admin.articles.create') }}" class="btn btn-sm btn-icon btn-outline-primary float-right">Create <span class="btn-inner--icon"><i class="fa fa-plus"></i></span></a>
                            ...
                            <form id="delete_all" class="d-none" action="{{route('admin.articles.delete.all')}}" method="post">
                                @csrf
                                <input type="hidden" class="article_ids" name="article_ids" value="">
                            </form>
                        </div>
                    </div>
                </div>
                                ...
                                <td class="text-right">
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                            <a class="dropdown-item has-icon" href="{{ route("admin.articles.edit", ['article' => $item]) }}"><i class="fa fa-edit"></i> Show & Edit</a>
                ...
                <div class="modal fade" id="delete_model_{{ $item->id }}" tabindex="-1" role="dialog" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <form action="{{ route('admin.articles.destroy', ['article' => $item]) }}" method="Post">
                                @method('DELETE')
                                @csrf
...
<script type="text/javascript">
    ...
    function setArticleId() {
        ...
        $('.notify_article_ids').val(str.slice(0, -1))
    }
</script>
@endsection
